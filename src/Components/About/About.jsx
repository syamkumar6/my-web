  ...
  return (
    <div id="about" className="lg:grid grid-cols-2 gap-4 w-full pt-8 bg-gray-100">
      ...
      <div className="col-span-2  p-4 flex flex-col justify-center items-center gap-4 mb-8">
        <h2 className="text-4xl font-customFont1 mt-8 text-gray-800 flex flex-col items-center">
            ABOUT ME
        ...
        <div className="px-8 py-3 m-">
          <h5 className="text-3xl font-customFont1 px-5 mb-8">Get to know me!</h5>
          <div className="px-5 flex flex-col gap-4">
            <p className="font-customFont2 text-gray-700">Hello there! I&apos;m a passionate and motivated <span className="font-bold text-lg">full-stack developer</span>  with a 
              flair for creating dynamic and interactive web applications. While I may not have formal 
              working experience in a corporate setting, I&apos;ve spent the last six months honing my skills in 
              full-stack development through an intensive course at <span className="font-bold text-lg">Entry elevate.</span>
            </p>
            <p className="font-customFont2 text-gray-700">During this transformative journey, I delved into the intricacies of both front-end and back-end 
            ...
            {showFullText && (
            <>
              <p className="font-customFont2 text-gray-700">
          ...
          {!showFullText && (
            <button
              ...
              className=" text-black border-b border-purple-500 self-end hover:bg-purple-600 duration-100 hover:text-white hover:border-none py-2 px-4 rounded-xl font-semibold"
            >
              Read more...
            </button>
          )}
          </div>
          <button className="ml-5 my-8 bg-purple-700 hover:bg-purple-600 text-white py-3 px-8 rounded-xl font-semibold">
            Contact</button>
        </div>

        <div className=" p-3">
          <h5 className="text-3xl font-customFont1 px-5 mb-8">My Skills</h5>
          <ul className="flex flex-wrap gap-3 px-5">
            {skills.map((skill, index) => {
              return <li key={index} className="px-3 py-2 bg-slate-200 rounded-md">
                {skill}
                </li>;
            })}
          </ul>
